...
| `gold_hud` | 170×42 | — | top=18, left=18 | `gold_hud.png`（42） | 金币胶囊（网页 `.gold-hud`），屏幕 (18,18) |
| `shop_btn` | 92×92 | — | left=36, bottom=36 | `shop_btn.png`（92） | 商店圆形按钮（🛒 已入图），屏幕左下 (36,36) |
| `open_btn` | 92×92 | — | right=36, bottom=36 | `open_btn.png`（92） | 背包圆形按钮（🧺 已入图），屏幕右下 (36,36) |
| `lb_退出登录` | 120×30 | (0, -330) | 无 | — | 纯文字按钮（Button Transition=NONE），屏幕 (580..700, 45..75) |
| `toast` | 480×44 | — | top=90, hCenter | `toast.png`（44） | Toast 容器，屏幕顶部居中 (400,90)，**最上层**（网页 z-index:30） |
...
|---|---|---|---|---|---|
| 底 | `panel` | 680×524 | (0,0) | `panel.png`（18） | 绿底(上#5a7a33→下#38521f) 金边(#d9b56a, 3px) 圆角18 + 内发光白6% |
| 1 | `header` | 680×56 | (0, **234**) | `panel_header.png`（[15,15,15,4]） | 顶部渐变条 + 底部 2px 金边；圆角贴合面板 |
| 2 | `toolbar` | 680×52 | (0, **180**) | `toolbar.png`（[2,2,2,2]） | 深色条 rgba(0,0,0,.22) + 底 1px 金边 |
| 3 | `scroll` | 680×372 | (0, **-32**) | — | `Mask`(GRAPHICS_RECT 矩形) + `ScrollView`(垂直滚动, inertia, elastic) |
| 4 | `footer` | 680×44 | (0, **-240**) | `panel_footer.png`（[2,2,2,2]） | 顶部 2px 金边 |
...
| `gold_hud.png` | 340×84 | 170×42 | 42 |
| `shop_btn.png` / `open_btn.png` | 184×184 | 92×92 | 92 |
| `toast.png` | 960×88 | 480×44 | 44 |
| `overlay_scrim.png` | 4×4 | 全屏 | 无（拉伸） |
| `panel.png` | 680×524 | 680×524 | 18 |
| `panel_header.png` | 680×56 | 680×56 | [15, 15, 15, 4] |
| `toolbar.png` | 680×52 | 680×52 | [2, 2, 2, 2] |
| `panel_footer.png` | 680×44 | 680×44 | [2, 2, 2, 2] |
| `tab_normal.png` / `tab_active.png` | 136×72 | 68×36 | 36 |
| `sort_normal.png` / `sort_active.png` | 152×64 | 76×32 | 28 |
| `close_btn.png` | 72×72 | 36×36 | 18 |
| `cell.png` | 400×400 | 97.33×97.33 | 24 |
| `sell_badge.png` | 112×44 | 56×22 | 16 |
| `buy_normal.png` / `buy_disabled.png` | 144×52 | 72×26 | 24 |

> 九宫格 inset 含义：图片四个边向内多少像素不可拉伸（保护圆角/边框）。
> **inset = 圆角半径（纹理像素）**；我们的图片用「外圈实心环」画边框，边框在圆角矩形**内部**，
> 已包含在半径保护区内，无需额外加边框宽度。
> 例如 `panel.png` inset=18 = 圆角半径 18；拉伸中间区域时四角不变形。
> 数组 [左, 上, 右, 下] 可分别设置（header 顶部圆角 15 / 底部 4）。
...
1. **数字单位是"纹理像素"（图片本身的像素），不是 UI 逻辑坐标**。
   例如 `setImg(node, 'tab_normal.png', 36)` 的 36 = 图片 136×72 里四边各向内 36px 不拉伸。

2. **为什么是 36 而不是 40？** 我们的图片按 **2x 绘制**（高清屏更清晰），
   且边框用「外圈实心环」画在圆角矩形**内部**，所以保护区 = 圆角半径本身：
   `slice = 2 × 圆角半径 = 2 × 18 = 36px`。✅
   （之前文档写的 40 是"圆角 + 边框"的过度保护：tab 高 72px，40+40=80 会超过图片高度，
   虽然引擎会自动 clamp 不至于画坏，但数值不严谨，已改为精确值 36。）
   同理 `panel.png` slice 18：1x 图，圆角半径 18，无需乘 2。

3. **什么时候有用？** 只有节点尺寸 ≠ 图片原始尺寸（被拉宽/拉高）时才需要。
   例如 panel 始终 680×524 时填不填都行；但如果哪天把面板拉宽到 800，
   有 slice=18 就能保证四角圆角和金边不变形，只有中间被拉伸。
...
6. **slice 传数组可以四边不同**：`setImg(node, 'panel_header.png', [15, 15, 15, 4])`
   = 左15 / 上15 / 右15 / 下4。header 顶部圆角 15 所以上边距 15，底部只有 2px 分隔线所以 4 就够。

7. **胶囊形（Tab / 金币HUD / Toast / 圆形按钮）的正确 slice = 半径，上下 inset 相加正好 = 图片高度**
   （如 tab：36+36=72）：垂直方向中间直段为 0，**不能垂直拉伸**——这恰恰是正确的，
   胶囊本来就不该被拉高；如果以后需要更高的胶囊，应重新生成图片而不是拉伸。

### 9.2 每张图的 slice 数值来源

**公式：`inset = 圆角半径（纹理像素）`** —— 2x 图 = 2×逻辑半径；1x 图 = 逻辑半径。
（边框画在圆角矩形内部，已包含在半径保护区内，无需再加。）

| 图片 | 纹理尺寸 | 圆角半径(纹理px) | slice = 半径 | 上下相加 vs 高度 |
|---|---|---|---|---|
| `tab_normal/active.png` | 136×72 (2x，半径18逻辑=36) | 36 | 36 | 36+36=72=高度 → 中间0（胶囊，垂直不可拉伸 ✅） |
| `sort_normal/active.png` | 152×64 (2x，半径14=28) | 28 | 28 | 28+28=56 < 64 → 中间8px ✅ |
| `gold_hud.png` | 340×84 (2x，半径21=42) | 42 | 42 | 42+42=84=高度 → 胶囊 ✅ |
| `toast.png` | 960×88 (2x，半径22=44) | 44 | 44 | 44+44=88=高度 → 胶囊 ✅ |
| `close_btn.png` | 72×72 (2x，半径9=18) | 18 | 18 | 18+18=36 < 72 ✅ |
| `cell.png` | 400×400 (2x，半径12=24) | 24 | 24 | 24+24=48 < 400 ✅ |
| `sell_badge.png` | 112×44 (2x，半径8=16) | 16 | 16 | 16+16=32 < 44 ✅ |
| `buy_normal/disabled.png` | 144×52 (2x，半径12=24) | 24 | 24 | 24+24=48 < 52 ✅ |
| `panel.png` | 680×524 (1x，半径18) | 18 | 18 | 18+18=36 < 524 ✅ |
| `panel_header.png` | 680×56 (1x，顶部圆角15) | 顶15/左右15 | [15, 15, 15, 4] | 底部只保护 2px 分隔线，4 够用 |
| `toolbar.png` / `panel_footer.png` | 680×52/44 (1x，无圆角) | 无 | [2, 2, 2, 2] | 只需保护 1~2px 边框线 |
| `shop_btn.png` / `open_btn.png` | 184×184 (2x，圆形半径92) | 92 | 92 | 92+92=184=尺寸 → 中间0（圆形不可拉伸 ✅） |

> 胶囊/圆形（Tab、金币HUD、Toast、圆形按钮）上下 inset 相加正好等于图片高度 → 中间直段为 0，
> 垂直方向**刻意不可拉伸**（胶囊/圆形拉高会变形，正确做法是重新生成图片）。
> 表格里「中间 0」正是精确值应有的结果，不是填错。
